...
// Bio-Adaptive Fitness RPG Engine v2.0
//
...
// Maps exercise primary_muscles strings → our 6 canonical groups.
...
function eModFromRIR(rir: number | null, setType: string): number {
  ...
  if (rir === null) return 0.9; // assume ~RIR 2 if untracked
  if (rir <= 1) return 1.0;
  if (rir === 2) return 0.9;
...
// Sigmoid normalization: 100 × (1 − e^(−stat/200))
// stat ≈ 45 after onboarding → score ≈ 20
// stat ≈ 200 after consistent training → score ≈ 63
// stat ≈ 400 elite → score ≈ 86
function statToScore(stat: number): number {
  return Math.min(100, Math.round(100 * (1 - Math.exp(-stat / 200))));
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // User-scoped client (validates session)
    ...
    // ── 2. Today's completed workout sets ─────────────────────────────────────
    ...
    // ── 3. Personal records → e1RM per exercise ───────────────────────────────
    ...
    const exerciseIds = (todayWorkouts ?? [])
    ...
    // ── 5. Sleep (most recent within 2 days) ──────────────────────────────────
    ...
    const rTotal = sleepMod * protMod;

    // ── 8. Current muscle stats ───────────────────────────────────────────────
    ...
    // ── 9. Initialize from onboarding strength test (first run only) ──────────
    ...
    if (!hasAnyStats) {
      ...
      if (initStr) {
        ...
        // Scale Sstart to stat units (×10 so sigmoid gives ~10-40% initial score)
        const SCALE = 10;
    ...
    // ── 10. Compute stimulus per muscle group ─────────────────────────────────
    ...
    for (const workout of todayWorkouts ?? []) {
      for (const we of workout.workout_exercises) {
        ...
        // Fallback: use exercise_target field when muscle data is missing.
        if (rawMuscles.length === 0 && we.exercise_target) {
          const targets = Array.isArray(we.exercise_target)
            ? we.exercise_target
            : [we.exercise_target];
        ...
        const groups = [...new Set(
          rawMuscles.map(mapMuscle).filter((g): g is string => g !== null)
        )];
        ...
        for (const set of we.workout_sets) {
          ...
          // Stset = ln(Load/e1RMactive × Reps × Emod + 1)
    ...
    // ── 11. Apply decay + growth (RPG engine) ─────────────────────────────────
    const GRACE_DAYS = 5;
    ...
    // ── 13. Body part scores (0–100) ──────────────────────────────────────────
    const bodyPartScores: Record<string, number> = {};
    for (const group of ALL_GROUPS) {
      bodyPartScores[group] = statToScore(newStatMap[group]);
    }

    // ── 14. Sub-scores ────────────────────────────────────────────────────────

    // Workout score: total_stimulus / expected_stimulus × 100
    const workoutScore = (todayWorkouts?.length ?? 0) > 0
    ...
    const qualityScore = sleepQuality === 'good' ? 100 : sleepQuality === 'okay' ? 60 : 20;
    ...
    // Mifflin-St Jeor BMR → TDEE → goal target
    ...
    // Total score: geometric mean of body part scores (The "Leg Day" Guard)
    ...
    const totalScore = geometricMean;

    // Balance score: penalises large disparities between muscle groups
    const avg = bpValues.reduce((s, v) => s + v, 0) / bpValues.length;
    const stdDev = Math.sqrt(bpValues.reduce((s, v) => s + (v - avg) ** 2, 0) / bpValues.length);
