...
export interface VoiceLogBodyProps {
  date: string;
  userId: string;
  onClose: () => void;
  onSaved: () => void;
...
/**
 * Voice meal logging body — the expo-audio–dependent UI.
 *
 * This module statically imports expo-audio (a native module), so it is loaded
 * LAZILY by VoiceLogSheet only when the sheet is opened. That keeps expo-audio
 * off the Logs-tab load path: an OTA can ship to any SDK 54 binary without
 * crashing builds that don't yet bundle the expo-audio native module — a failed
 * load degrades to an error fallback instead.
 *
 * Mounted only while the sheet is visible (the wrapper unmounts it on close),
 * so component state resets naturally between sessions.
 */
export default function VoiceLogBody({ date, userId, onClose, onSaved }: VoiceLogBodyProps) {
  ...
  // Stop any in-progress recording when the body unmounts (sheet closed).
  useEffect(() => {
